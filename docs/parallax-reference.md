# Parallax scroll component — reference

Saved for later use. This is the original snippet Cam provided (Osmo-style layered
parallax driven by GSAP ScrollTrigger + Lenis smooth scroll).

## What it needs before it works
- **Deps:** `gsap` (with `gsap/ScrollTrigger`) and `@studio-freight/lenis`.
- **CSS:** none is included here. The `parallax__*` classes need styles that:
  absolutely-stack the layers, set the header section height (e.g. 100vh), clip
  overflow, center the layer images, and fade the bottom edge. Osmo's own
  stylesheet is not part of this snippet.
- **Images:** the `src` URLs point at Osmo's CDN demo art (a generic 3D scene).
  For production, copy real/brand layers into `public/` and reference locally.
- **Smooth scroll caveat:** `Lenis` hijacks scrolling for the WHOLE site, not just
  this component. It can affect sticky-nav anchor jumps and accessibility. Only
  enable it globally on purpose.
- **Reduced motion:** wrap the animation in a `prefers-reduced-motion` check
  before shipping.
